...
: New Python syntax and functions introduced in Lab 04
...
: The dwfpy device model; this lab's new instruments in bold

{{< pagebreak >}}

## Laboratory Introduction

In Labs 02 and 03 you measured signals the world handed you — a pendulum swung, and you recorded what its sensors did. This week you take control of the *source* side of the measurement chain: you will **generate** signals with known frequency, amplitude, and offset, and then study how faithfully a sampled measurement reproduces them.

The scientific focus of this lab is **sampling theory.** A DAQ cannot observe a signal continuously; it records snapshots at a sample rate, $f_s$. If those snapshots are too far apart, they cannot uniquely capture a rapidly changing signal: different real signals can produce the same sequence of measurements. You have already selected sample rates twice this course (100 Hz in Lab 02 and 1000 Hz in Lab 03), mostly by trusting the manual. This week, you will learn the rule behind that choice—and see what happens when it is broken. A fast signal can appear in your data as a slower signal that does not actually exist. This deceptive effect, called **aliasing**, is a common source of distortion in sampled data: it can produce strange sounds in audio, visual artifacts in images, or misleading vibration frequencies when measuring a beam, motor, or rotating machines. By the end of the session, you will see an LED blinking rapidly while your data incorrectly reports that it blinked only once every two seconds.

Along the way you will also:

- run a **two-instrument showdown** (handheld DMM vs. scripted ADS acquisition) on known DC levels, sharpening your understanding of accuracy/precision/resolution;
- write Python that drives a **digital output pin**, the simplest computer control of the physical world, and discover why software loops make poor clocks;
...
where $A$ is the **amplitude**, $f$ the **frequency** in Hz, and $V_{offset}$ is the **DC offset**. Two derived quantities appear constantly on instrument panels: the **peak-to-peak voltage** $V_{pp} = 2A$ and the **period** $T = 1/f$. @fig-signal-anatomy labels all of them on this lab's standard test signal ($V_{offset} = 2.5$ V, $A = 2$ V), which conveniently spans 0.5–4.5 V — safely inside the ADS's range, and always positive.
...
A DAQ (**d**ata **aq**uisition system) samples a continuous signal at discrete instants $t_k = k/f_s$, producing snapshots $y(t_k)$ and discarding everything in between. The measured signal is useful is it is captured "fast enough." But, how fast is fast enough? The answer to this question is the **Nyquist criterion**: a sampled signal can only faithfully represent frequencies below the **Nyquist frequency**,
...
A blinking LED looks *steady* if it blinks fast enough. The changeover frequency — the **flicker-fusion threshold** — is typically 50–90 Hz, varying with brightness, contrast, where the image lands on your retina (peripheral vision is more flicker-sensitive), and from person to person. Above the threshold your visual system fuses the flicker into steady light, but the flicker is still physically there — and "invisible" flicker from cheap LED lighting and displays is a documented cause of eyestrain and headaches, which is why lighting standards (e.g., IEEE 1789) regulate it. In Part-6 you will measure your own threshold, using the Wavegen as a precision flicker source. Your eye is not literally a discrete sampler like the ADS, but it *is* a measurement system with finite bandwidth (that is the range of frequency in which works as desired!) — and one key takeaway the lab is that you can find and understand the bandwidth of every instrument you use, including the two built into your head.

{{< pagebreak >}}

## Part-1: Build the LED Circuits {#sec-part-1}

Use these figures @fig-led-schematic and the annotated photo in @fig-circuit-build as guides to assist in building your circuit.

![The breadboard build: both LED branches and the Channel 1 tap. (Note in the figure above 5V is connected to test LEDs, but you don't need these connections. ](figures/LED_Circuit_Build_ADS.png){#fig-circuit-build width="100%"}
...
2.  Check each LED with your DMM in **diode-test mode**: probes matching the LED's polarity should light it faintly and read $V_f$; reversed, it should read over-limit.
3.  Nothing should be lit yet — W1 and DIO 0 both idle at 0 V.

::: {.callout-important title="Logbook Questions"}
**Q.** Record the forward voltage $V_f$ your DMM's diode test reports for each LED. Using @eq-led-current, what current will flow in the W1 branch when W1 outputs 5 V? When DIO 0 outputs its 3.3 V HIGH?

**Q.** In one sentence: what would happen to the LED if you omitted the 330 Ω resistor, and *why* doesn't the LED protect itself?
:::
...
Now using a new instrument, we are going to follow the same rule as Lab 03: **explore it live in the GUI before you automate it**. The Wavegen is the ADS's built-in signal generator. It is useful for generating precise controllable electrical signals useful for testing circuits.
...
5.  Now raise the frequency: 2 Hz, 5Hz, 10 Hz, 20, Hz, 30 Hz, 40 Hz, then 50 Hz, then 100 Hz. Watch the LED at each step and note what your eyes report.
6.  Switch the waveform to **Square** and try all listed frequencies above. How do these change?
7.  When you are done exploring, **close WaveForms fully** (check the system tray) so your scripts can claim the device.

![Wavegen configured for the standard test signal.](figures/Wavegen_Sine_Setup.png){#fig-wavegen-setup width="100%"}

::: {.callout-important title="Logbook Questions"}
**Q.** Record the cursor-measured period at 1 Hz. Does it match $1/f$? What is the *expected* period at 100 Hz?

**Q.** Describe what the LED looked like at each frequency. At what frequency (roughly) did it stop looking like a blinking light with the sine wave? How about with the square wave?
:::
...
The ADS accepts one controlling program at a time. Close WaveForms fully before running dwfpy cells.
:::
...
- **`wavegen['ch1'].setup('dc', offset=v_set, start=True)`** — the Wavegen as code. `'dc'` is a waveform *function* just like `'sine'`; a DC "waveform" ignores frequency and amplitude and simply holds the `offset` voltage. `start=True` makes the output live immediately — equivalent to clicking **Run** in the GUI. Note that `wavegen['ch1']` is the GUI's **W1**, the same convention as `scope['ch1']`.
- **`float(input(...))`** — `input()` always returns *text*, even if you type `3.302`. Wrapping it in `float(...)` converts that text to a number you can do math with. Type only the number — no units.
- Notice the pattern: one `with` block, *two* instruments (`wavegen` and `scope`) work together in this one. This source-plus-measurement pairing is the skeleton for many automated test scripts.
...
::: {.callout-important title="Logbook Questions"}
**Q.** Copy your six-row comparison table into your logbook. Which instrument's errors are larger? Which instrument's errors follow a *pattern* (a trend with voltage), and which look random? What does a trend suggest about the *type* of error?

**Q.** What is the resolution (smallest displayed step) of your DMM? Estimate the ADS's effective resolution from the digits that change between repeated captures. Resolution, precision, accuracy: which of the three does averaging 2000 samples improve, and which can it never improve?

**Q.** The "error" here is measured against the W1 *setpoint* — but W1 is itself an instrument with its own tolerance. What third piece of equipment would you need to decide which of the three (W1, DMM, ADS) is closest to the truth?
:::
...
In this part, you will generate five sine frequencies — chosen relative to the fixed Nyquist frequency $f_N = 500$ Hz — capture each at $f_s = 1000$ Hz, and compare what you *generated* against what the data *shows*.

### Predict First

Good experimentalists commit predictions before looking at data. Compute the apparent frequency @eq-folding predicts for every sweep frequency:
...
**`round()`** is a Python built-in: `round(1.125)` → 1, so for $f_{signal} = 1125$ Hz the formula computes $|1125 - 1 \times 1000| = 125$ Hz. Note this whole cell is pure arithmetic so you can run it *before* capturing any data.

::: {.callout-important title="Logbook Questions"}
**Q.** Write the five predictions as a table in your logbook **before running the sweep**: $f_{signal}$, predicted $f_{apparent}$, and one short phrase of what you expect the plot to look like. Check each against @fig-folding.
:::
...
- **`time.sleep(0.2)`** pauses the script for 0.2 seconds — here, giving the Wavegen a beat to settle on its new frequency before the capture starts. `sleep` is the function for "wait a moment" in instrument scripts.
...
- The five captures land as five CSV files with `#`-header lines — load them with `np.loadtxt(..., comments='#')`.

While it runs, glance at the LED: at these frequencies it looks steadily lit even as the *data* is about to tell five very different stories.

### Plot All Five Panels

A shared figure with stacked panels is a standard way to compare captures, and it introduces one new tool:
...
- **`plt.subplots(5, 1, figsize=(6.5, 10.0))`** creates one figure holding a 5-row, 1-column grid of axes; `axes` is an array you can loop over. Everything you know about formatting a single `ax` applies unchanged to each panel.
- **`zip(axes, f_signals, observed, x_max_ms)`** — `zip`, now pairs *four* lists at once. One loop pass = one fully-labeled panel.
- Run it once with the `'?'` placeholders, then **measure the observed frequency in each panel** — read the time between successive peaks off the axis and invert it — and re-run with `observed` filled in (e.g., `'450 Hz — aliased!'`). The observed values are *your measurement*; the title's predicted value is the theory we should compare too.

Format per the Post-Lab requirements and save **.pdf**/**.png** at 600 DPI. Compare with @fig-example-alias.

::: {.callout-important title="Logbook Questions"}
**Q.** For each panel: did the observed frequency match your Q8 prediction? Record the observed values in your Q8 table.

**Q.** Compare panels 2 (450 Hz) and 4 (550 Hz) closely. Could you tell, from the sampled data alone, which input produced which panel? Explain why this means an anti-aliasing filter must act *before* the sampler.

**Q.** In panel 3 (exactly $f_N$), what amplitude does the sampled signal show? Re-run the sweep and watch whether it changes. What does the sampled amplitude at exactly $f_N$ depend on?

**Q.** In Labs 02 and 03 you sampled a swinging pendulum at 100 Hz and 1000 Hz. Estimate the highest frequency present in those signals, and argue from @eq-nyquist whether aliasing could have corrupted your measurements.
:::
...
::: {.callout-important title="Logbook Questions"}
**Q.** LED 2 runs at 3.3 V, LED 1 at up to 5 V. Which is brighter, and does the difference match the current ratio predicted by @eq-led-current using your Q1 values?

**Q.** Name one lab task where software timing (a `sleep` loop) is perfectly fine, and one where it would ruin the measurement. What is the rough timescale that separates them?
:::
...
::: {.callout-important title="Logbook Questions"}
**Q.** What frequency does your captured data show? What did your eyes see during those same 20 seconds? Reconcile the two using @eq-folding.

**Q.** A colleague argues: "the data is clean and periodic — clearly the LED blinked every \~2 seconds." Nothing in the file itself contradicts them. What *outside knowledge* is required to catch the error, and what does that imply about trusting logged data from a system whose sample rate you didn't choose?
:::
...
::: {.callout-important title="Logbook Questions"}
**Q.** Record both partners' threshold brackets. Are they the same? Typical values run 50–90 Hz — where do you two fall?

**Q.** Re-run a frequency just *above* your threshold, but this time look slightly *away* from the LED, watching it with your peripheral vision. Does the flicker come back? (For most people it does.) What does that tell you about treating "the eye" as one single sensor?

**Q.** LED room lights are typically dimmed by switching on/off rapidly (PWM). Using your measured threshold plus the safety callout above, propose a minimum PWM frequency for comfortable, safe lighting, and justify it in two sentences.
:::
