...
%% Sample data acquistion program for collecting the dynamics data and store 
% file for later use 
% Date: July 19th
% Dr. Vibhav Durgesh
% Rev 0.0
...
T  = ; % ADD APPROPRIATE SAMPLE TIME TO COLLECT 4-5 PEAKS IN THE SIGNAL
filename = 'DynSignal_975Hz.dat'; % To store experiment data
fid1 = fopen(filename,'w');
plotTitle = 'Your Name''s plot - Experiment#4';
fprintf(fid1,'%s \t %s \n','time (s)', 'Voltage (V)');
...
voltData = event.Data; %Potentiometer data in voltage
figure(1)
plot(time, voltData,'k.-');hold on
xlabel('time (s)')
ylabel('Votlage (v)')
end
