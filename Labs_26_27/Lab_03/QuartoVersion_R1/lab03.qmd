...
- Measure **two sensors at once**: the Lab 02 potentiometer plus a MEMS accelerometer, acquired on two differential channels
- Write your **first Python automation** of the ADS using `pydwf`, replacing a repetitive GUI task (a multi-point calibration) with a script
- **Calibrate a MEMS accelerometer** using gravity as the reference, via a four-orientation static calibration
...
- Why can gravity serve as a calibration reference for an accelerometer?
...
This lab introduces `pydwf`, the Python interface to the WaveForms SDK. Add it to your course environment: open a terminal in your `ME3300` folder and run
...
Read the [Background](#sec-background) section before lab. It explains what the accelerometer measures, derives the gravity-correction equation you will implement, and introduces the pydwf device model.

### Complete the Prelab Walkthrough Notebook {#sec-prelab-walkthrough}

Download `Lab03_Prelab_Walkthrough.ipynb` from Canvas into `ME3300/Lab_03/Code/` and work through it before lab. It introduces this lab's *new* Python skills using simulated signals (no hardware needed at home):
...
As always, report the **checkpoint** values in the **Prelab 03 quiz on Canvas** before your lab session. Skills from Labs 01–02 (loading CSVs, `polyfit`, fit statistics, `for`/`enumerate`, f-strings, `.iloc`, plot formatting) are not retaught.
...
In Lab 02 you completed the full measurement chain for one sensor: build, verify, calibrate, measure, model. This lab scales that chain up in three directions at once.

- **Two sensors, two channels.** You will measure the pendulum's angle (potentiometer, Channel 1) and the acceleration at its tip (MEMS accelerometer, Channel 2) *simultaneously*. Multichannel measurement is the norm in real experiments — one signal alone rarely tells the whole story.
- **Your first DAQ automation.** The accelerometer calibration requires holding the pendulum in four orientations and recording each one. In Lab 02 you did this kind of repetitive record–export–rename loop by hand, eleven times. This week, a short `pydwf` script does the repetition for you: it prompts you for each position, records, and averages, all in one run. This is the course's automation philosophy from here forward: **use the GUI where it makes sense** (one-off captures, live exploration) **and Python where tasks repeat**. By Lab 10 you will be able to script complete experiments — a skill that carries directly into capstone and industry work.
- **Independent cross-checks.** You will determine the pendulum tip's normal acceleration two completely independent ways — measured directly by the accelerometer, and calculated from the differentiated potentiometer angle — and compare them. Agreement between independent methods is one of the strongest forms of experimental evidence; their *disagreement* teaches you about noise, alignment, and modeling assumptions.
...
where $\dot\theta$ is the angular velocity in rad/s. The minus sign reflects our sign convention: the accelerometer's **+y axis points from the sensor toward the pivot** (see @fig-orientations), and the centripetal acceleration a swinging pendulum tip experiences points that same way. You will measure $\dot\theta$ by numerically differentiating the potentiometer angle, and $r$ with a tape measure (pivot to accelerometer center).

### What the Accelerometer Measures

The MEMS accelerometer (MMA7361, mounted at the pendulum tip; see @fig-accel-board) contains a microscopic proof mass on silicon springs. It measures **proper acceleration** — the deflection of that mass — which includes gravity. A stationary accelerometer with its +y axis pointing straight up reads +9.81 m/s², not zero, because the springs must hold the proof mass up against gravity.

For our swinging pendulum, the y-axis therefore senses *both* effects at once:

$$a_y = \underbrace{-\dot{\theta}^2 r}_{\text{normal accel.}} \; \underbrace{- \, g\cos\theta}_{\text{gravity component}}$$ {#eq-accel-reading}

where $\theta$ is the pendulum angle measured from straight down. The gravity term follows from the free-body diagram in @fig-gravity-fbd: the component of $g$ along the +y direction (tip toward pivot) is $-g\cos\theta$ — largest in magnitude when hanging straight down ($\theta = 0$), zero when horizontal. In Part 5 you will *subtract* this term from the measured signal to isolate the normal acceleration and enable a fair comparison with @eq-normal-accel.

![Free-body diagram of the accelerometer at angle $\theta$. The +y axis points from the sensor toward the pivot; the gravity component along +y is $-g\cos\theta$.](figures/Accel_Gravity_FBD.png){#fig-gravity-fbd width="45%"}

### Gravity as a Calibration Reference

Equation @eq-accel-reading also hands us a free calibration standard. When the pendulum is held *still*, $\dot\theta = 0$ and the sensor feels only the gravity component — which we know exactly at four easy-to-set orientations:
...
is appropriate — now mapping volts to m/s² instead of degrees. Four points spanning ±1 g are enough to pin down the line, and the fit-quality statistics are computed exactly as in Lab 02 (norm of residuals, $s_{yx}$, 95% CI, $S_{a_1}$ — see Lab 02 Eqs. 7–9), just with different units.

### From GUI to Code: the pydwf Device Model

`pydwf` exposes every WaveForms instrument as a Python object (see the quick-reference table above). Setting a sample rate in code, `ai.frequencySet(1000)`, does *exactly* what typing 1000 Hz into the GUI does — same instrument, same hardware, different interface. Because you drove these instruments by hand in Lab 02, none of this is magic; it is the same knobs, turned programmatically. One consequence: **only one program can hold the device at a time**, so WaveForms must be closed before a pydwf script runs (and vice versa).
...
![The MMA7361 accelerometer board mounted at the pendulum tip, with its 3-wire cable.](figures/Accel_Board_Photo.png){#fig-accel-board width="70%"}
