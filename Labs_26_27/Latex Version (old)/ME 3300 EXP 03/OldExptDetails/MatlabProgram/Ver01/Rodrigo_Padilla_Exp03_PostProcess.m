...
fid = fopen('Expt_03_Firstname_Lastname.dat'); % Your data file
linel = fgetl(fid);
data = fscanf(fid, '%f \n', [3 inf]);
x = data(1,:); % Time data
y1 = data(2,:); % Angular location (degrees)
y2 = data(3,:); % Accelerometer data (m/s^2)
fclose(fid);

rad = y1*pi/180; %convert degree values to radians
theta_d =  diff(rad)./ diff(x); % calculating angular velcoity rad/s 

acc_1 =  - (theta_d.^2) * r; %calculated acceleration m/s^2
acc_2 = y2 - (g*cos(rad)); % removing acceleration due to gravity component
% accelerometer data
