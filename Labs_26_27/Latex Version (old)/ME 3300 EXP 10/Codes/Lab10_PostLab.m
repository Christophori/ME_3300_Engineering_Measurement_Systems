...
phi = delay.*2*pi.*f*(180/pi); % degrees
phaseshift_exp = - phi;
phaseshift_theo = -atan(2*pi*f*tau)*(180/pi);

figure(2)
set(gcf,'unit','inches','position',[0.50 0.50 6.50 3.50],...
        'defaultaxesfontsize',10,'defaultaxesfontname','times');
semilogx(tau_omega,phaseshift_exp,'ko','markerfacecolor','k');hold on
semilogx(tau_omega,phaseshift_theo,'b-','markerfacecolor','b','linewidth',2)
xlabel('\tau \omega')
