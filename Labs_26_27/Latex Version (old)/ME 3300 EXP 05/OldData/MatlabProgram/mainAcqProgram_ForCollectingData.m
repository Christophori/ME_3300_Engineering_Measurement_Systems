...
slope = 1; % Add your calibration slope for pressure transducer
intercept = 0; % Add your calibration intercept for pressure transducer
v0 = 0.00; % Voltage output from sensor at zero flow rate.
...
flowrate = (slope*(event.Data-v0))+intercept; %Flow rate
figure(1)
plot(time, flowrate (scfm),'k.-');hold on
xlabel('time (s)')
ylabel('Flowrate(scfm)')
end

function logData(src, evt, fid,slope,intercept,v0);
% Add the time stamp and the data values to data. To write data sequentially,
% transpose the matrix.
data = [evt.TimeStamps evt.Data (slope*(evt.Data-v0))+intercept] ;
fprintf(fid,'%f \t %f \t %f\n',data');
end